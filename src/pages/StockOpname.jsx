...
function StockOpname() {
  ...
  const filteredProducts = productList.filter((product) => {
    ...
    return (matchProduct || matchVariant) && matchSessionMode
  })
    ...
    <MainLayout>
      <div className="min-h-screen">
        <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm font-semibold text-slate-500">
              Buat sesi SO, cek stok fisik per ukuran, dan pantau hasil opname
              toko.
            </p>
          </div>
        ...
        <div className="mb-5 grid gap-3 md:grid-cols-4">
          ...
          <SummaryCard
            label="Item SO"
            value={stockOpnameHistory.length}
            color="amber"
          />
        </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-5 grid gap-4 xl:grid-cols-[1fr_520px] xl:items-start">
            <div>
              <h2 className="text-xl font-black text-slate-900">
                Daftar Barang untuk SO
              </h2>

              <p className="mt-1 text-sm font-semibold text-slate-400">
                Pilih barang, buka detail ukuran, lalu input stok fisik dengan
                tombol SO.
              </p>
            </div>
            ...
            <input
              ...
              placeholder="Cari barang, SKU, barcode, ukuran, rak..."
          ...
          {filteredProducts.length === 0 ? (
            <EmptyState text="Barang untuk stock opname tidak ditemukan" />
          ) : (
                  ...
                  return (
                    <div key={product.id} className="bg-white">
                      <div className="grid gap-4 px-4 py-4 xl:grid-cols-[1.4fr_0.7fr_0.8fr_0.7fr_0.7fr_auto] xl:items-center">
                        ...
                        >
                          {isExpanded ? "Tutup" : "Detail SO"}
                        </button>
                      </div>

                      {isExpanded && (
                        <StockOpnameProductDetail
                          product={product}
...
function StockOpnameProductDetail({
  product,
  formatRupiah,
    ...
    <div className="border-t border-slate-100 bg-slate-50 px-4 py-4">
      <div className="mb-3">
        <p className="text-sm font-black text-slate-800">
          Input SO Per {product.variantType || "Varian"}
        </p>
        <p className="text-xs font-semibold text-slate-400">
          Klik tombol SO pada ukuran yang ingin dicek stok fisiknya.
        </p>
      </div>
